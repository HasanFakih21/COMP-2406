...
let students = [
	...
	let firstNames = students.map(student => student.fname);
	console.log(firstNames);

	let fullNames =  students.filter(student => student.egrade>=80);
	fullNames = fullNames.map(student => student.fname + " " + student.lname);
	console.log(fullNames);
	
	let grades = students.map(student => (student.agrade * 0.4) + (student.tgrade * 0.1) + (student.egrade * 0.5));
	grades = grades.reduce((total, grade) => (total + grade));
	console.log("The average grade of all students is: " + grades/students.length);
...
function setList() {
	res1.innerHTML = restaurants[0].name;
	res2.innerHTML = restaurants[1].name;
	res3.innerHTML = restaurants[2].name;
}

function render() {
	let body = "";
	for(let i = 0; i<2;i++) {
		let temp = restaurants[1].menu[i];
		body += '<option>' + temp + '</option>';
	}
	submenu.innerHTML =  body;
}